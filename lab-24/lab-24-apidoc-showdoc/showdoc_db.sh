...
#
#
host="120.55.163.126"				#数据库所在地址。默认是localhost
port=3306						#数据库所在端口。默认是3306
user="root"   	 			#数据库的用户名
password="FW#Server" 			#密码
db="bpm_ha" 					#要同步的数据库名。要同步多个db可以将本脚本复制多份
api_key="25f4e7fdf1a2c33342b63c44478ee17418563990" 			#api_key
api_token="30d68b0a268640b581643d9257155629176553235" 	#api_token
cat_name="数据字典" 	#可选。如果想把生成的文档都放在项目的子目录下，则这里填写子目录名。
url="https://192.168.177.4:4999/server/?s=/api/open/updateDbItem" #可选。同步到的url。如果是使用www.showdoc.com.cn ，则不需要再改此项。如果是部署私有版版showdoc，请改此项为http://xx.com/server/index.php?s=/api/open/updateDbItem 。其中xx.com为你的部署域名
#
#
#
#
