...
###### `nexus3 （端口：8081，账户：admin，密码：nexus@123）`

`1panel （端口：36138，账户：root1panel，密码：1panel123456）`
...
```linux
docker run -d -p 8081:8081 --privileged=true --name nexus3 \
-v /soft/nexus3/data:/nexus-data sonatype/nexus3
```
...
###### `consul（tcp端口：8300，ui端口：8500）、`

###### `elastic-job-lite(端口：8088，账户：root，密码：root)`
...
###### `activeMQ（Web Console 端口：8161，账户：admin，密码：admin，Broker 服务端口：61616）、`
...
###### `Soul Admin（端口：9095，账户：admin，密码：123456,目前该网关技术已被 Apache ShenYu 所取代）`

###### `Soul-bootstrap（网关服务，端口：9195）`

###### `Shenyu Admin（端口：9095，账户：admin，密码：123456）`

###### `Shenyu-bootstrap（网关服务，端口：9195）`
...
###### `cat（端口：2280，http 端口：8080，账户：admin，密码：admin）`
...
```linux
  重启：cd /soft/harbor/ && docker-compose down
	   ./prepare && docker-compose up -d
  停止：cd /soft/harbor/ && docker-compose stop
```
...
```linux
  启动：nohup ./consul agent -dev  -client 0.0.0.0 -ui &
```

###### `elastic-job-lite`

```linux
  启动：nohup sh bin/start.sh &
  停止：sh bin/stop.sh
```
...
```linux
  ...
  启动Broker：nohup sh bin/mqbroker -c conf/broker.conf  -n 192.168.177.4:9876 &
...
```linux
  启动：bin/linux-x86-64/activemq start/restart
  日志：tail -f data/wrapper.log
```
...
```linux
  启动：nohup java -jar soul-admin.jar（废弃，因为存在bug）
```

###### `Soul-bootstrap`

```linux
  启动：nohup java -jar soul-bootstrap.jar --soul.sync.websocket.url="ws://192.168.177.4:9095/websocket" > nohup-soul-bootstrap.out &
```
...
```linux
  启动：nohup java -jar zipkin-server.jar &
```

###### `cat`

```linux
  启动：sh apache-tomcat-8.0.53/bin/startup.sh
  停止：sh apache-tomcat-8.0.53/bin/shutdown.sh
```

###### `wrk`

```linux
  压测: ./wrk -t50 -c400 -d30s url
```
...
```linux
  alertmanager  http://192.168.177.5:8080
  grafana  http://192.168.177.5:8081   默认用户名 admin  默认密码 admin
  prometheus http://192.168.177.5:8082
  node_export http://192.168.177.5:9100
  dashboard http://192.168.177.5:10000
```
...
```linux
   k8s dashboard: 30080
   查看 secret: kubectl -n kubernetes-dashboard describe secret $(kubectl -n kubernetes-dashboard get secret | grep admin-user | awk '{print $1}')
   部署情况: master 192.168.177.6 
            slaver 192.168.177.7 192.168.177.8
```
